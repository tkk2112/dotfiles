...
def get_repo_states(module):
    rc, out, err = module.run_command(
        [DNF_BIN, "repolist", "--all", "--verbose"], check_rc=True
    )
    ...
    for line in out.split("\n"):
        ...
        if m:
            if len(last_repo) > 0:
                module.fail_json(
                    msg="dnf repolist parse failure: parsed another repo id before next status"
                )
            last_repo = m.group(1)
            continue
        ...
        if m:
            if len(last_repo) == 0:
                module.fail_json(
                    msg="dnf repolist parse failure: parsed status before repo id"
                )
...
def set_repo_states(module, repo_ids, state):
    module.run_command(
        [DNF_BIN, "config-manager", "--assumeyes", f"--set-{state}"] + repo_ids,
        check_rc=True,
    )
...
def main():
    ...
    for repo_id in names:
        if repo_id not in repo_states:
            module.fail_json(
                msg=f"did not find repo with ID '{repo_id}' in dnf repolist --all --verbose"
            )
    ...
    for repo_id in to_change:
        if repo_states_post[repo_id] != desired_repo_state:
            module.fail_json(
                msg=f"dnf config-manager failed to make '{repo_id}' {desired_repo_state}"
            )
